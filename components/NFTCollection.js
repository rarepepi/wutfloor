...
const NFTCollection = (props) => {
    const url = `https://opensea.io/collection/${}`;
            ...
            <Fade>
                ...
                <div className="flex flex-col justify-center text-center text-white w-64 border-2 border-purple-500 rounded-xl lg:pt-8 pt-4">
                    ...
                    <div className="relative text-xs mb-8 mt-2 text-purple-400 cursor-pointer" onMouseEnter={() => setTooltipStatus(1)} onMouseLeave={() => setTooltipStatus(0)}>
                        {tooltipStatus == 1 && (
                            <div role="tooltip" className="z-20 w-54 -top-96 mt-2 absolute transition duration-150 ease-in-out shadow-lg bg-white p-4 border-2 border-purple-400 rounded bg-background bg-opacity-100">
                                <p className="text-xs ">Days Deployed <span className="mb-2 text-green-200 block" >{parseInt((new Date() - new Date(props.asset.created_date)) / (1000 * 3600 * 24))}</span></p>
                                ...
                                <p className="text-xs text-purple-400">Total Volume <span className="block text-green-200"  ><img src="https://storage.opensea.io/files/6f8e2979d428180222796ff4a33ab929.svg" className="h-4 w-4 inline mr-2" />{(Math.round(props.asset.total_volume * 100) / 100).toLocaleString()}</span><span className="block text-green-200 mb-4"  >(${(Math.round((props.asset.total_volume * props.ethPrice) * 100) / 100).toLocaleString()})</span></p>
                    ...
                    <div className="divide-y-4 divide-solid divide-purple-500 divide-opacity-75 space-y-2">
                        ...
                        <p className="text-xs p-1 text-gray-400">1 Day Avg Price <span className="text-green-200 block mt-1"><img src="https://storage.opensea.io/files/6f8e2979d428180222796ff4a33ab929.svg" className="h-4 w-4 inline mr-2" />{Math.round(props.asset.one_day_average_price * 100) / 100}</span></p>
                        <p className="text-xs p-1 text-gray-400">7 Day Avg Price <span className="text-green-200 block mt-1" ><img src="https://storage.opensea.io/files/6f8e2979d428180222796ff4a33ab929.svg" className="h-4 w-4 inline mr-2" />{Math.round(props.asset.seven_day_average_price * 100) / 100}</span></p>
                        <p className="text-xs p-1 text-gray-400">30 Day Avg Price <span className="text-green-200 block mt-1" ><img src="https://storage.opensea.io/files/6f8e2979d428180222796ff4a33ab929.svg" className="h-4 w-4 inline mr-2" />{Math.round(props.asset.thirty_day_average_price * 100) / 100}</span></p>
                    </div>
                </div>
            </Fade >
        </div >
